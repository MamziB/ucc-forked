...
do {                                                                      \
    _max_count = 2 << (TL_MLX5_MCAST_IB_IMMEDIATE_PACKET_BIT_COUNT -      \
                    ucc_ilog2(ONE_SIDED_RELIABILITY_MAX_TEAM_SIZE) -      \
                    ucc_ilog2(ONE_SIDED_MAX_ALLGATHER_COUNTER));          \
} while (0);
...
ucc_status_t ucc_tl_mlx5_mcast_allgather_init(ucc_tl_mlx5_task_t *task)
{
    ...
    MCAST_GET_MAX_ALLGATHER_PACKET_COUNT(comm->allgather_comm.max_num_packets);
