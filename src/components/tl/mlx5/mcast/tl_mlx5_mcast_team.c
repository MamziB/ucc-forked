                                         ...
                                         ucc_tl_mlx5_mcast_coll_comm_init_spec_t *mcast_conf)
{
    ...
    comm->allgather_post = ucc_tl_mlx5_mcast_service_allgather_post;
    comm->barrier_post   = ucc_tl_mlx5_mcast_service_barrier_post;
    comm->coll_test      = ucc_tl_mlx5_mcast_service_coll_test;
    ...
    if (comm->rank == 0) {
        switch(tl_team->mcast_state) {
            ...
            case TL_MLX5_TEAM_STATE_MCAST_GRP_BCAST_POST:
            {
                ...
                status = comm->coll_test(comm->group_setup_info_req);
                if (UCC_OK != status) {
