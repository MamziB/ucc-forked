...
static ucc_status_t ucc_tl_mlx5_mcast_service_bcast_post(void *arg, void *buf, int size, int root,
...
ucc_status_t ucc_tl_mlx5_mcast_team_post(ucc_base_context_t *base_context,
                                         ...
                                         ucc_tl_mlx5_mcast_coll_comm_init_spec_t *mcast_conf)
{
    ...
    new_mcast_team = ucc_calloc(1, sizeof(ucc_tl_mlx5_mcast_team_t),
                                "new_mcast_team");
    ...
    comm = (ucc_tl_mlx5_mcast_coll_comm_t*)
            ucc_malloc(sizeof(ucc_tl_mlx5_mcast_coll_comm_t) +
...
static ucc_status_t ucc_tl_mlx5_mcast_coll_setup_comm_resources(ucc_tl_mlx5_mcast_coll_comm_t *comm)
{
    ucc_status_t status;
    size_t       page_size;
    int          buf_size, i;
    ...
    if (!comm->pp_mr) {
        tl_error(comm->lib, "could not register pp_buf mr, errno %d", errno);
        status = UCC_ERR_NO_MEMORY;
        goto error;
    }
    ...
    if (comm->rank == 0) {
        switch(tl_team->mcast_state) {
            ...
            case TL_MLX5_TEAM_STATE_MCAST_GRP_JOIN_READY:
            {
                ...
                mgid_s = sizeof(tmp.param.ud.ah_attr.grh.dgid);
                mlid_s = sizeof(tmp.param.ud.ah_attr.dlid);
                ...
                memcpy((void*)((ptrdiff_t)data + sizeof(ucc_status_t)),
                        &comm->event->param.ud.ah_attr.grh.dgid, mgid_s);
                memcpy((void*)((ptrdiff_t)data + sizeof(ucc_status_t) + mgid_s),
                ...
                comm->mcast_lid  = *((uint16_t*)((ptrdiff_t)data + sizeof(ucc_status_t) +
                                     mgid_s));
                comm->mcast_addr = net_addr;

                memcpy((void*)&comm->mgid, (void*)((ptrdiff_t)data + sizeof(ucc_status_t)),
                       mgid_s);
        ...
        switch(tl_team->mcast_state) {
            ...
            case TL_MLX5_TEAM_STATE_MCAST_GRP_BCAST_POST:
            {
                ...
                memcpy(&net_addr.sin6_addr, (void*)((ptrdiff_t)data +
                            sizeof(ucc_status_t)), sizeof(struct in6_addr));
                net_addr.sin6_family = AF_INET6;

                status = ucc_tl_mlx5_mcast_join_mcast_post(comm->ctx, &net_addr, 0);
                if (status < 0) {
                    tl_error(comm->lib, "none-root rank is unable to join mcast group");
                    ucc_free(data);
                    return status;
                }
            ...
            case TL_MLX5_TEAM_STATE_MCAST_GRP_JOIN_POST:
            {
                ...
                comm->mcast_lid  = *((uint16_t*)((ptrdiff_t)comm->group_setup_info +
                            sizeof(ucc_status_t) + mgid_s));
                comm->mcast_addr = net_addr;

                memcpy((void*)&comm->mgid, (void*)((ptrdiff_t)comm->group_setup_info +
                            sizeof(ucc_status_t)), mgid_s);
