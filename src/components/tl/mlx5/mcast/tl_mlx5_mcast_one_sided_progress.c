...
static inline ucc_status_t ucc_tl_mlx5_mcast_drain_recv_wr(mcast_coll_comm_t *comm,
                                                                      ...
                                                                      int *completed)
{
    ...
    assert(comm->enable_truly_zero_copy_pipelined_allgather);
    assert(sched);
    assert(req->allgather_rkeys_req == NULL);
    assert(!comm->rdma_read_in_progress);
    ...
    for (j = 0; j < comm->mcast_group_count; j++) {
        if (comm->pending_recv_per_qp[j] != 0) {
            ...
            assert(comm->pending_recv >= 0 && req->to_recv >= 0);
        }
    }
    ...
    assert(!comm->pending_recv);

    for (j = 0; j < req->concurreny_level; j++) {
        root = sched[req->step].multicast_op[j].root;
        // comm->recv_list[root] will not be incremented if there is out of
        // order packet
        if (comm->recv_list[root] != sched[req->step].multicast_op[j].to_recv) {
...
ucc_status_t ucc_tl_mlx5_mcast_reliable_one_sided_get(mcast_coll_comm_t *comm,
                                                      mcast_coll_req_t  *req,
                                                      int *completed)
{
    ucc_status_t             status;
    ...
    assert(!(ONE_SIDED_SYNCHRONOUS_PROTO == req->one_sided_reliability_scheme &&
           comm->rdma_read_in_progress));

    for (target = 0; target < comm->commsize; target++) {

        if (comm->recv_list[target] != req->num_packets) {

           tl_trace(comm->lib, "%d of the packets from source target %d are dropped",
                    req->num_packets - comm->recv_list[target], target);
           ...
           switch(req->one_sided_reliability_scheme) {
                ...
                case ONE_SIDED_ASYNCHRONOUS_PROTO:
                     ...
                     * completed once all the values in recv_list[] is set to req->num_packets
                     * and comm->pending_reads is set to 0 */

                    if (req->ag_counter == comm->remote_slot_info[target]) {
                        ...
                        remote_addr = comm->one_sided_async_slots_info_list[target].remote_addr
                                      + (req->ag_counter % ONE_SIDED_SLOTS_COUNT) * comm->one_sided_async_slots_size
                        ...
                        comm->pending_reads++;
                        target_completed++;
                        comm->remote_slot_info[target] = ONE_SIDED_PENDING_DATA;
                        comm->recv_list[target]        = req->num_packets;

                    } else if (ONE_SIDED_PENDING_INFO != comm->remote_slot_info[target] && 
                               ONE_SIDED_PENDING_DATA != comm->remote_slot_info[target]) {
                        /* remote slot is not valid yet. Need to do an rdma 
                         * read to check the latest state */

                        src_addr    = &comm->remote_slot_info[target];
                        remote_addr = comm->one_sided_async_slots_info_list[target].remote_addr
                                      + (req->ag_counter % ONE_SIDED_SLOTS_COUNT) * comm->one_sided_async_slots_size;
                        lkey        = comm->remote_slot_info_mr->lkey;
                        rkey        = comm->one_sided_async_slots_info_list[target].rkey; 
                        size        = ONE_SIDED_SLOTS_INFO_SIZE;
                        wr          = MCAST_AG_RDMA_READ_INFO_WR;

                        comm->remote_slot_info[target] = ONE_SIDED_PENDING_INFO;
...
ucc_status_t ucc_tl_mlx5_mcast_progress_one_sided_communication(mcast_coll_comm_t *comm, mcast_coll_req_t *req)
{
    ...
    if (!comm->enable_truly_zero_copy_pipelined_allgather && !req->to_send &&
            !req->to_recv) {
        ...
        case ONE_SIDED_SYNCHRONOUS_PROTO:
            ...
            if (!comm->pending_reads) {
                ...
                if (!comm->enable_truly_zero_copy_pipelined_allgather &&
                    !comm->enable_truly_zero_copy_allgather) {
                    req->to_recv = 0;
                }
                                                         ...
                                                         int coll_type)
{
    ...
    assert(pp->context == 0); // make sure it's a recv packet not send

    // process the immediate value saved in pp->psn
    if (UCC_COLL_TYPE_BCAST != coll_type) {
     ...
     if (ag_counter == (req->ag_counter % ONE_SIDED_MAX_ALLGATHER_COUNTER)) {
        switch(coll_type) {
            case UCC_COLL_TYPE_ALLGATHER:
                if (comm->enable_truly_zero_copy_allgather ||
                        comm->enable_truly_zero_copy_pipelined_allgather) {
                    ...
                    if (comm->enable_memcopy_offload) {
                        /* nonblocking write the received mcast packet into the receive buffer of myself */
                        ...
                        target      = comm->rank;
                        src_addr    = (void*)pp->buf;
                        remote_addr = dest;
                        rkey        = req->recv_mr->rkey;
                        lkey        = comm->pp_mr->lkey;
                        size        = pp->length;
                        wr          = (uint64_t)pp;

                       status = ucc_tl_one_sided_p2p_put(src_addr, remote_addr, size, lkey, rkey, target, wr, comm);
                       if (UCC_OK != status) {
                            return status;
                       }

                       comm->pending_memcopy_offload++;
                        
                       while(comm->params.sx_depth / 2 < comm->pending_memcopy_offload) {
                            if (ucc_tl_mlx5_mcast_poll_send(comm) < 0) {
                                return UCC_ERR_NO_MESSAGE;
                            }
                       }
                    } else {
                        if (comm->cuda_enabled) {
            ...
            case UCC_COLL_TYPE_ALLREDUCE:
                if (pp->length) {
                    if (pp->length == comm->max_per_packet) {
                        dest = req->rptr + offset * pp->length;
                    } else {
                        dest = req->rptr + (req->length - pp->length);
                    }

                    count = pp->length / ucc_dt_size(req->dt);

                    if (comm->cuda_enabled) {
                        return UCC_ERR_NOT_IMPLEMENTED;
                    } else {
                        assert(NULL != req->executor);
                        status = ucc_dt_reduce((void*) pp->buf, dest, dest, count, req->dt, req->args, 0, 0,
                                               req->executor, &req->etask);
                        if (ucc_unlikely(status != UCC_OK)) {
                            tl_error(comm->lib, "failed to perform dt reduction");
                            return status;
                        }

                        assert(NULL != req->etask);

                        do {
                            status = ucc_ee_executor_task_test(req->etask);
                        ...
                        ucc_ee_executor_task_finalize(req->etask);
                    }
                }
            ...
            case UCC_COLL_TYPE_BCAST:
                ...
                if (offset != pp->packet_counter) {
                    /* recevied out of order packet */
                    tl_error(comm->lib, "recevied out of order: packet counter %d recv size counter %d",
                             offset, pp->packet_counter);
        ...
        if (!(comm->mcast_trigger_reliability && offset == PACKET_TO_DROP)) {
            ...
            if (comm->one_sided_reliability_enabled) {
                ...
                if (out_of_order_recvd == 0) {
                    comm->recv_list[source_rank]++;
                }

                if (comm->recv_list[source_rank] > req->num_packets) {
                    tl_error(comm->lib, "reliablity failed: comm->recv_list[%d] %d"
                            " req->num_packets %d offset %d PACKET_TO_DROP %d"
                            " comm->ag_under_progress_counter %d req->ag_counter"
                            " %d \n", source_rank, comm->recv_list[source_rank],
            ...
            //assert(comm->pending_recv_per_qp[pp->qp_id] >= 0);

        } else {
            
            tl_debug(comm->lib, "dropped packet %d intentionally to test reliablity",
                    PACKET_TO_DROP);

        }

        if (!comm->enable_memcopy_offload) {
            if (comm->one_sided_reliability_enabled && comm->enable_truly_zero_copy_pipelined_allgather) {
        ...
        assert(comm->one_sided_reliability_enabled);
        pp->context = 0;
        ucc_list_add_tail(&comm->bpool, &pp->super);
    }
}
